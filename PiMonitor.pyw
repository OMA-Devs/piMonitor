...
class Aplicacion():
	...
	def __init__(self):
		...
		self.control = SpeedController(12)
		##################
		##LOGOS y TITULO##
		##################
	...
	def PID(self):
		...
		## Cuando el bucle lee los valores, puede darse que esten vacios. En ese caso, se revierte a los
		## valores por defecto.
		try:
			self.minTEMP = int(self.minVAR.get())
			self.maxTEMP = int(self.maxVAR.get())
		except:
			self.minTEMP = 30
			self.maxTEMP = 70
			self.minVAR.set(30)
			self.maxVAR.set(70)
		self.log.info("PID()- MIN: "+str(self.minVAR.get())+"| MAX: "+str(self.maxVAR.get())+"| ACT: "+str(temp))
